...
int main() {
	...
	while (true)
	{
		...
		cvtColor(cFrame, HSVFrame, COLOR_BGR2HSV);
		// threshold matrix
		...
		// ?
		trackFilteredObject(x, y, thresholdFrame, cFrame);

		// Show frames
		imshow(cameraWindowName, cFrame);
		imshow(HSVWindowName, HSVFrame);
		imshow(thresholdWindowName, thresholdFrame);

		cv::waitKey(30);	// 一個frame所顯示的milisecond
	}


	std::cout << "Hello";	// 我不知道這行output到哪裡去了
...
//{
//
//	argv[1] = "C:/Users/yckuo/Desktop/maxresdefault.jpg";
//
//	Mat image;
//
//
